...
namespace WowPacketParserModule.V5_3_0_16981.Parsers
{
    public static class TradeHandler
    {
        ...
        public static void HandleTradeStatus(Packet packet)
        {
            ...
            var status = packet.Translator.ReadBitsE<TradeStatus530>("Status", 5);
            switch (status)
            {
                case TradeStatus530.BeginTrade:
                    var guid = packet.Translator.StartBitStream(0, 3, 2, 4, 1, 6, 7, 5);
                    packet.Translator.ParseBitStream(guid, 5, 7, 3, 6, 4, 2, 0, 1);
