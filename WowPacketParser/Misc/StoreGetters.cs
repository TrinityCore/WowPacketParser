        ...
        public static string GetName(StoreNameType type, int entry, bool withEntry = true)
        {
            ...
            switch (type)
            {
                ...
                case StoreNameType.Model:
                    if (DBC.DBC.CreatureDisplayInfo.ContainsKey((uint)entry))
                    {
                        string modelText = string.Empty;

                        for (var i = 0; i < 3; ++i)
                        {
                            if (DBC.DBC.CreatureDisplayInfo[(uint)entry].TextureVariation[i] != string.Empty)
                            {
                                modelText += modelText == string.Empty ? $"TextureVariation: " +
                                    $"{DBC.DBC.CreatureDisplayInfo[(uint)entry].TextureVariation[i]}" :
                                    $" / {DBC.DBC.CreatureDisplayInfo[(uint)entry].TextureVariation[i]}";
                            }
                        }

                        if (DBC.DBC.CreatureDisplayInfo[(uint)entry].PortraitTextureName != string.Empty)
                            modelText += $"PortraitTextureName: {DBC.DBC.CreatureDisplayInfo[(uint)entry].PortraitTextureName}";

                        if (modelText == string.Empty)
                            modelText = entry.ToString();
