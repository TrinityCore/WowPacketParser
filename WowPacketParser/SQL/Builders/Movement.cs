        ...
        public static string MovementData()
        {
            ...
            if (Settings.TargetedProject == TargetedProject.Cmangos)
            {
                ...
                foreach (var data in Storage.CreatureMovement)
                {
                    ...
                    output += $"INSERT INTO waypoint_path_name(PathId, Name) VALUES('@MOVID + {pathIdCounter}','{SQLUtil.EscapeString(StoreGetters.GetName(Utilities.ObjectTypeToStore(creatureMovement.GUID.GetObjectType()), (int)creatureMovement.GUID.GetEntry()))}');\n";
                    output += nodeSql;

                    int pointIdCounter = 1;
                    foreach (var node in creatureMovement.Waypoints)
                    {
                        bool finalP = creatureMovement.Destination == null && creatureMovement.Waypoints.Count != pointIdCounter;
