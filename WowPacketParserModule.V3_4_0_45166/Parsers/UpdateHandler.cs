...
namespace WowPacketParserModule.V3_4_4_59817.Parsers
{
    public static class UpdateHandler
    {
        ...
        [Parser(Opcode.SMSG_UPDATE_OBJECT, ClientVersionBuild.V3_4_4_59817)]
                    ...
                    case UpdateTypeCataclysm.Values:
                    {
                        ...
                        using (var fieldsData = new Packet(packet.ReadBytes((int)updatefieldSize), packet.Opcode, packet.Time, packet.Direction, packet.Number, packet.Writer, packet.FileName))
                        {
                            ...
                            if (objectIndirectFragment >= 0 && changedFragments[objectIndirectFragment])
                            {
                                ...
                                {
                                    var updateTypeFlag = (uint)fieldsData.ReadUInt32E<UpdateTypeFlag>("UpdateTypeFlag", i);
                            ...
                            if (fieldsData.Position != fieldsData.Length)
                            {
                                ...
                                fieldsData.WriteLine("UpdateField UpdateBlock not fully read! Current position: {0} Length: {1} Bytes remaining: {2}.",
                                    pos, len, len - pos);
                            }
                        }
