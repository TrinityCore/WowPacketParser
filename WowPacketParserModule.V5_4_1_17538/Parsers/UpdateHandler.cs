    ...
    public static class UpdateHandler
    {
        ...
        private static MovementInfo ReadMovementUpdateBlock(Packet packet, WowGuid guid, object index)
        {
            ...
            if (isLiving)
            {
                ...
                if (hasTransportData)
                {
                    packet.Translator.StartBitStream(transportGuid, 4, 7, 3, 1, 6);
                    hasTransportTime2 = packet.Translator.ReadBit();
                    packet.Translator.StartBitStream(transportGuid, 2, 0, 5);
            ...
            {
                packet.Translator.StartBitStream(goTransportGuid, 0, 7);
                ...
                packet.Translator.StartBitStream(goTransportGuid, 6, 5, 4, 3, 2);
            }
            ...
            if (hasAttackingTarget)
                packet.Translator.StartBitStream(attackingTargetGuid, 5, 4, 6, 0, 1, 7, 2, 3);
            ...
            if (hasSceneObjectData)
            {
                ...
                for (var i = 0; i < 2; ++i)
                {
                    ...
                    packet.Translator.StartBitStream(guid358[i], 3, 4, 1, 0);
                    bits358_10[i] = packet.Translator.ReadBits(2);
                    bit358_20[i] = !packet.Translator.ReadBit();
                    ...
                    for (var j = 0; j < bits358_10[i]; ++j)
                    {
                        ...
                        packet.Translator.StartBitStream(guid358_6[i][j], 5, 3, 4, 1, 0, 6, 7);
                        bits358_6_31[i][j] = packet.Translator.ReadBits(7);
                        guid358_6[i][j][2] = packet.Translator.ReadBit();
                    ...
                    guid358[i][6] = packet.Translator.ReadBit();
                    bit358_C[i] = !packet.Translator.ReadBit();
                    packet.Translator.StartBitStream(guid358[i], 2, 5);
                ...
                packet.Translator.StartBitStream(CreatorGUID, 2, 7, 0, 5, 1, 3, 6, 4);
            }
            ...
            if (hasSceneObjectData)
            {
                for (var i = 0; i < 2; ++i)
                {
                    for (var j = 0; j < bits358_10[i]; ++j)
                    {
                        ...
                        packet.Translator.ReadXORBytes(guid358_6[i][j], 5, 3);
                    ...
                    packet.Translator.ReadXORBytes(guid358[i], 0, 7);
                    packet.Translator.ReadByte("byte358+25", index, i);
                    if (bit358_24[i])
                        packet.Translator.ReadInt32("int358+C", index, i);

                    packet.Translator.ReadXORBytes(guid358[i], 5, 4);
                    if (!bit358_20[i])
                    ...
                    packet.Translator.ReadXORBytes(guid358[i], 1, 2, 3, 6);
                ...
                packet.Translator.ParseBitStream(CreatorGUID, 3, 5, 4, 2, 7, 6, 0, 1);
            ...
            if (isLiving)
            {
                ...
                if (hasTransportData)
                {
                    ...
                    packet.Translator.ReadXORBytes(transportGuid, 4, 7);
                    ...
                    packet.Translator.ReadXORBytes(transportGuid, 3, 6);
                ...
                packet.Translator.ReadXORBytes(guid1, 7, 2);
                moveInfo.Position.Z = packet.Translator.ReadSingle();
            ...
            if (hasGameObjectPosition)
            {
                ...
                packet.Translator.ReadXORBytes(goTransportGuid, 4, 2, 7, 3);
                ...
                moveInfo.TransportOffset.O = packet.Translator.ReadSingle();
                packet.Translator.ReadXORBytes(goTransportGuid, 5, 0);
                moveInfo.TransportOffset.X = packet.Translator.ReadSingle();
            ...
            if (hasAttackingTarget)
            {
                packet.Translator.ParseBitStream(attackingTargetGuid, 4, 6, 3, 0, 7, 1, 2, 5);
            ...
            if (isLiving && moveInfo.HasSplineData && hasFullSpline && splineType == SplineType.FacingTarget)
            {
                ...
                facingTargetGuid = packet.Translator.StartBitStream(5, 3, 6, 2, 7, 0, 1, 4);
                packet.Translator.ParseBitStream(facingTargetGuid, 7, 0, 1, 4, 2, 5, 3, 6);
