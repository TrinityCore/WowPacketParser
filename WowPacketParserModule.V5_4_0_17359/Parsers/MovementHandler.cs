using WowPacketParser.Enums;
using WowPacketParser.Misc;
using WowPacketParser.Parsing;
using CoreParsers = WowPacketParser.Parsing.Parsers;

namespace WowPacketParserModule.V5_4_0_17359.Parsers
{
    public static class MovementHandler
    {

        [Parser(Opcode.SMSG_LOGIN_SET_TIME_SPEED)]
        public static void HandleUnknown431(Packet packet)
        {
            packet.Translator.ReadSingle("Game Speed");
            packet.Translator.ReadPackedTime("Game Time");
            packet.Translator.ReadInt32("Unk 1");
            packet.Translator.ReadInt32("Unk 2");
            packet.Translator.ReadPackedTime("Game Time?");
        }

        [Parser(Opcode.SMSG_LOGIN_VERIFY_WORLD)]
        public static void HandleLoginVerifyWorld(Packet packet)
        {
            var pos = new Vector4();

            pos.Z = packet.Translator.ReadSingle();
            packet.Translator.ReadInt32<MapId>("Map");
            pos.Y = packet.Translator.ReadSingle();
            pos.X = packet.Translator.ReadSingle();
            pos.O = packet.Translator.ReadSingle();

            packet.AddValue("Position", pos);
        }

        [Parser(Opcode.SMSG_BIND_POINT_UPDATE)]
        public static void HandleBindPointUpdate(Packet packet)
        {
            var pos = new Vector3();

            pos.Y = packet.Translator.ReadSingle();
            pos.X = packet.Translator.ReadSingle();
            pos.Z = packet.Translator.ReadSingle();
            packet.Translator.ReadInt32<AreaId>("Area Id");
            CoreParsers.MovementHandler.CurrentMapId = (uint)packet.Translator.ReadInt32<MapId>("Map");

            packet.AddValue("Position", pos);
        }

        [Parser(Opcode.SMSG_ON_MONSTER_MOVE)]
        public static void HandleMonsterMove(Packet packet)
        {
            var ownerGUID = new byte[8];
            var guid2 = new byte[8];
            var factingTargetGUID = new byte[8];
            var pos = new Vector3();

            var bitB0 = packet.Translator.ReadBit();
            ownerGUID[5] = packet.Translator.ReadBit();
            var hasAnimationTime = !packet.Translator.ReadBit();
            var hasTime = !packet.Translator.ReadBit();
            packet.Translator.StartBitStream(ownerGUID, 4, 3);
            var bit4C = !packet.Translator.ReadBit();
            var bit78 = packet.Translator.ReadBit();
            ownerGUID[2] = packet.Translator.ReadBit();
            var hasFlags = !packet.Translator.ReadBit();
            ownerGUID[0] = packet.Translator.ReadBit();

            var bits8C = 0u;
            if (bitB0)
            {
                packet.Translator.ReadBits("bits9C", 2);
                bits8C = packet.Translator.ReadBits(22);
            }

            var waypointCount = packet.Translator.ReadBits(22);
            var bit6D = !packet.Translator.ReadBit();
            ownerGUID[7] = packet.Translator.ReadBit();
            packet.Translator.ReadBit(); // fake bit
            packet.Translator.StartBitStream(guid2, 5, 3, 4, 6, 2, 1, 7, 0);
            var hasAnimationState = !packet.Translator.ReadBit();
            var hasParabolicTime = !packet.Translator.ReadBit();
            var hasParabolicSpeed = !packet.Translator.ReadBit();
            ownerGUID[6] = packet.Translator.ReadBit();
            var splineCount = packet.Translator.ReadBits(20);
            ownerGUID[1] = packet.Translator.ReadBit();
            var bit6C = !packet.Translator.ReadBit();
            var splineType = packet.Translator.ReadBits(3);

            if (splineType == 3)
                packet.Translator.StartBitStream(factingTargetGUID, 4, 6, 5, 1, 0, 7, 3, 2);

            packet.Translator.ReadBit("bit38");
            packet.Translator.ResetBitReader();

            if (bitB0)
            {
                packet.Translator.ReadSingle("floatA8");
                packet.Translator.ReadSingle("floatA0");
                packet.Translator.ReadInt16("shortAC");
                for (var i = 0; i < bits8C; ++i)
                {
                    packet.Translator.ReadInt16("short94+2", i);
                    packet.Translator.ReadInt16("short94+0", i);
                }
                packet.Translator.ReadInt16("shortA4");
            }

            packet.Translator.ReadXORBytes(guid2, 0, 1, 2, 7, 3, 4, 6, 5);
            if (splineType == 3)
            {
                packet.Translator.ParseBitStream(factingTargetGUID, 2, 1, 7, 0, 5, 3, 4, 6);
                packet.Translator.WriteGuid("Facting Target GUID", factingTargetGUID);
            }

            pos.Y = packet.Translator.ReadSingle();
            packet.Translator.ReadXORByte(ownerGUID, 7);
            if (hasAnimationTime)
                packet.Translator.ReadInt32("Asynctime in ms"); // Async-time in ms

            var waypoints = new Vector3[waypointCount];
            for (var i = 0; i < waypointCount; ++i)
            {
                var vec = packet.Translator.ReadPackedVector3();
                waypoints[i].X = vec.X;
                waypoints[i].Y = vec.Y;
                waypoints[i].Z = vec.Z;
            }

            packet.Translator.ReadSingle("Float2C");
            pos.Z = packet.Translator.ReadSingle();
            packet.Translator.ReadSingle("Float30");

            if (splineType == 2)
            {
                var spot = new Vector3
                {
                    Y = packet.Translator.ReadSingle(),
                    Z = packet.Translator.ReadSingle(),
                    X = packet.Translator.ReadSingle()
                };
                packet.AddValue("Facing Spot", spot);
            }

            Vector3 endpos = new Vector3();
            for (var i = 0; i < splineCount; ++i)
            {
                var spot = new Vector3
                {
                    Y = packet.Translator.ReadSingle(),
                    Z = packet.Translator.ReadSingle(),
                    X = packet.Translator.ReadSingle()
                };
                // client always taking first point
                if (i == 0)
                {
                    endpos = spot;
                }

                packet.AddValue("Spline Waypoint", spot, i);
            }

            packet.Translator.ReadXORByte(ownerGUID, 5);
            packet.Translator.ReadInt32("Move Ticks");

            if (hasFlags)
                packet.Translator.ReadInt32E<SplineFlag434>("Spline Flags");

            if (hasAnimationState)
                packet.Translator.ReadByteE<MovementAnimationState>("Animation State");

            packet.Translator.ReadXORByte(ownerGUID, 0);
            if (bit6D)
                packet.Translator.ReadByte("byte6D");

            if (hasParabolicTime)
                packet.Translator.ReadInt32("Async-time in ms");

            pos.X = packet.Translator.ReadSingle();
            if (hasTime)
                packet.Translator.ReadInt32("Move Time in ms");

            packet.Translator.ReadXORByte(ownerGUID, 4);
            if (hasParabolicSpeed)
                packet.Translator.ReadSingle("Vertical Speed");

            if (!bit78)
                packet.Translator.ReadByte("byte78");

            if (splineType == 4)
                packet.Translator.ReadSingle("Facing Angle");

            if (bit6C)
                packet.Translator.ReadByte("byte6C");

            packet.Translator.ReadSingle("Float34");
            if (bit4C)
                packet.Translator.ReadInt32("int4C");

            packet.Translator.ReadXORBytes(ownerGUID, 6, 2, 3, 1);

            // Calculate mid pos
            var mid = new Vector3();
            mid.X = (pos.X + endpos.X) * 0.5f;
            mid.Y = (pos.Y + endpos.Y) * 0.5f;
            mid.Z = (pos.Z + endpos.Z) * 0.5f;
            for (var i = 0; i < waypointCount; ++i)
            {
                var vec = new Vector3
                {
                    X = mid.X - waypoints[i].X,
                    Y = mid.Y - waypoints[i].Y,
                    Z = mid.Z - waypoints[i].Z
                };
                packet.AddValue("Waypoint", vec, i);
            }

            packet.Translator.WriteGuid("Owner GUID", ownerGUID);
            packet.Translator.WriteGuid("GUID2", guid2);
            packet.AddValue("Position", pos);
        }

        [Parser(Opcode.SMSG_PHASE_SHIFT_CHANGE)]
        public static void HandlePhaseShift(Packet packet)
        {
            CoreParsers.MovementHandler.ActivePhases.Clear();
            packet.Translator.ReadUInt32("UInt32 1");
            //packet.Translator.ReadUInt32("UInt32 2");

            var count = packet.Translator.ReadUInt32() / 2;
            packet.AddValue("WorldMapArea swap count", count);
            for (var i = 0; i < count; ++i)
                packet.Translator.ReadUInt16("WorldMapArea swap", i);

            count = packet.Translator.ReadUInt32() / 2;
            packet.AddValue("Phases count", count);
            for (var i = 0; i < count; ++i)
                CoreParsers.MovementHandler.ActivePhases.Add(packet.Translator.ReadUInt16("Phase id", i)); // Phase.dbc

            count = packet.Translator.ReadUInt32() / 2;
            packet.AddValue("Inactive Terrain swap count", count);
            for (var i = 0; i < count; ++i)
                packet.Translator.ReadInt16<MapId>("Inactive Terrain swap", i);


            count = packet.Translator.ReadUInt32() / 2;
            packet.AddValue("Active Terrain swap count", count);
            for (var i = 0; i < count; ++i)
                packet.Translator.ReadInt16<MapId>("Active Terrain swap", i);

            var guid = packet.Translator.StartBitStream(0, 2, 1, 5, 3, 7, 4, 6);
            packet.Translator.ParseBitStream(guid, 0, 5, 4, 7, 6, 2, 1, 3);
            packet.Translator.WriteGuid("GUID", guid);
        }

        [Parser(Opcode.SMSG_MOVE_UPDATE)]
        public static void HandlePlayerMove(Packet packet)
        {
            var guid = new byte[8];
            var transportGUID = new byte[8];
            var pos = new Vector4();

            var bit95 = packet.Translator.ReadBit();
            var hasPitch = !packet.Translator.ReadBit();
            guid[4] = packet.Translator.ReadBit();
            guid[2] = packet.Translator.ReadBit();
            var bitAC = packet.Translator.ReadBit();
            var hasFallData = packet.Translator.ReadBit();
            guid[7] = packet.Translator.ReadBit();
            var counter2 = (int)packet.Translator.ReadBits(22);
            guid[5] = packet.Translator.ReadBit();
            guid[3] = packet.Translator.ReadBit();
            var hasTimeStamp = !packet.Translator.ReadBit();
            var hasTransportData = packet.Translator.ReadBit();
            var hasTransportTime2 = false;
            var hasTransportTime3 = false;

            if (hasTransportData)
            {
                packet.Translator.StartBitStream(transportGUID, 1, 2, 3, 4, 5);
                hasTransportTime3 = packet.Translator.ReadBit();
                hasTransportTime2 = packet.Translator.ReadBit();
                packet.Translator.StartBitStream(transportGUID, 0, 7, 6);
            }

            var hasMovementFlags = !packet.Translator.ReadBit();

            if (hasMovementFlags)
                packet.Translator.ReadBitsE<MovementFlag>("Movement Flags", 30);

            var hasOrientation = !packet.Translator.ReadBit();
            var isAlive = !packet.Translator.ReadBit();
            var hasFallDirection = false;
            if (hasFallData)
                hasFallDirection = packet.Translator.ReadBit();

            var hasExtraMovementFlags = !packet.Translator.ReadBit();
            guid[6] = packet.Translator.ReadBit();
            guid[0] = packet.Translator.ReadBit();
            guid[1] = packet.Translator.ReadBit();
            var bit94 = packet.Translator.ReadBit();

            if (hasExtraMovementFlags)
                packet.Translator.ReadBitsE<MovementFlagExtra>("Extra Movement Flags", 13);

            var hasSplineElevation = !packet.Translator.ReadBit();

            pos.X = packet.Translator.ReadSingle();

            if (hasFallData)
            {
                if (hasFallDirection)
                {
                    packet.Translator.ReadSingle("Fall Sin");
                    packet.Translator.ReadSingle("Horizontal Speed");
                    packet.Translator.ReadSingle("Fall Cos");
                }

                packet.Translator.ReadInt32("Fall Time");
                packet.Translator.ReadSingle("Velocity Speed");
            }

            packet.Translator.ReadXORByte(guid, 3);

            if (hasTransportData)
            {
                var transPos = new Vector4();

                packet.Translator.ReadXORByte(transportGUID, 2);
                packet.Translator.ReadXORByte(transportGUID, 0);
                packet.Translator.ReadXORByte(transportGUID, 5);
                packet.Translator.ReadByte("Seat");
                packet.Translator.ReadXORByte(transportGUID, 4);
                packet.Translator.ReadXORByte(transportGUID, 3);
                if (hasTransportTime2)
                    packet.Translator.ReadInt32("Transport Time 2");

                packet.Translator.ReadXORByte(transportGUID, 6);
                packet.Translator.ReadXORByte(transportGUID, 7);
                transPos.X = packet.Translator.ReadSingle();
                if (hasTransportTime3)
                    packet.Translator.ReadInt32("Transport Time 3");

                packet.Translator.ReadInt32("Transport Time");
                transPos.Z = packet.Translator.ReadSingle();
                packet.Translator.ReadXORByte(transportGUID, 1);
                transPos.Y = packet.Translator.ReadSingle();
                transPos.O = packet.Translator.ReadSingle();

                packet.Translator.WriteGuid("Transport Guid", transportGUID);
                packet.AddValue("Transport Position", transPos);
            }

            packet.Translator.ReadXORByte(guid, 2);
            packet.Translator.ReadXORByte(guid, 6);

            for (var i = 0; i < counter2; ++i)
                packet.Translator.ReadInt32("Int9C", i);

            packet.Translator.ReadXORByte(guid, 1);

            if (hasPitch)
                packet.Translator.ReadSingle("Pitch");

            pos.Y = packet.Translator.ReadSingle();
            pos.Z = packet.Translator.ReadSingle();

            packet.Translator.ReadXORByte(guid, 4);

            if (isAlive)
                packet.Translator.ReadInt32("time(isAlive)");

            if (hasSplineElevation)
                packet.Translator.ReadSingle("Spline Elevation");

            if (hasTimeStamp)
                packet.Translator.ReadInt32("Timestamp");

            packet.Translator.ReadXORByte(guid, 0);
            packet.Translator.ReadXORByte(guid, 5);
            packet.Translator.ReadXORByte(guid, 7);

            if (hasOrientation)
                pos.O = packet.Translator.ReadSingle();

            packet.Translator.WriteGuid("Guid", guid);
            packet.AddValue("Position", pos);
        }

        [Parser(Opcode.CMSG_MOVE_TIME_SKIPPED)]
        public static void HandleMoveTimeSkipped434(Packet packet)
        {
            packet.Translator.ReadUInt32("Time");
            var guid = packet.Translator.StartBitStream(3, 0, 5, 1, 7, 6, 4, 2);
            packet.Translator.ParseBitStream(guid, 1, 6, 0, 5, 3, 4, 2, 7);
            packet.Translator.WriteGuid("Guid", guid);
        }

        [Parser(Opcode.SMSG_MOVE_UPDATE_RUN_SPEED)]
        public static void HandleMoveUpdateRunSpeed(Packet packet)
        {
            var guid = new byte[8];
            var transportGuid = new byte[8];
            var pos = new Vector4();

            var hasTransportTime3 = false;
            var hasTransportTime2 = false;

            guid[7] = packet.Translator.ReadBit();

            var hasPitch = !packet.Translator.ReadBit();

            guid[0] = packet.Translator.ReadBit();
            var hasFallData = packet.Translator.ReadBit();
            packet.Translator.StartBitStream(guid, 1, 4, 5);

            var hasMovementFlags = !packet.Translator.ReadBit();

            var hasFallDirection = false;
            if (hasFallData)
                hasFallDirection = packet.Translator.ReadBit();

            guid[2] = packet.Translator.ReadBit();

            packet.Translator.ReadBit("Has spline data");

            var hasMovementFlagsExtra = !packet.Translator.ReadBit();
            var hasTransport = packet.Translator.ReadBit();
            var hasSplineElevation = !packet.Translator.ReadBit();
            var bitA8 = !packet.Translator.ReadBit();
            var hasTime = !packet.Translator.ReadBit();

            if (hasTransport)
            {
                hasTransportTime3 = packet.Translator.ReadBit();
                hasTransportTime2 = packet.Translator.ReadBit();
                packet.Translator.StartBitStream(transportGuid, 4, 5, 3, 2, 7, 0, 1, 6);
            }

            var bit95 = packet.Translator.ReadBit();

            if (hasMovementFlagsExtra)
                packet.Translator.ReadBitsE<MovementFlagExtra>("Extra Movement Flags", 13);

            if (hasMovementFlags)
                packet.Translator.ReadBitsE<MovementFlag>("Movement flags", 30);

            var hasO = !packet.Translator.ReadBit();

            guid[3] = packet.Translator.ReadBit();

            var counter = packet.Translator.ReadBits(22);
            var bit94 = packet.Translator.ReadBit();

            guid[6] = packet.Translator.ReadBit();
            packet.Translator.ReadXORByte(guid, 2);
            packet.Translator.ReadXORByte(guid, 4);

            if (hasTransport)
            {
                var tpos = new Vector4();

                packet.Translator.ReadXORByte(transportGuid, 7);

                if (hasTransportTime3)
                    packet.Translator.ReadUInt32("Transport Time 3");

                tpos.O = packet.Translator.ReadSingle();

                packet.Translator.ReadXORByte(transportGuid, 1);
                packet.Translator.ReadXORByte(transportGuid, 6);
                packet.Translator.ReadXORByte(transportGuid, 0);

                if (hasTransportTime2)
                    packet.Translator.ReadUInt32("Transport Time 2");

                packet.Translator.ReadSByte("Transport Seat");
                tpos.Z = packet.Translator.ReadSingle();
                tpos.Y = packet.Translator.ReadSingle();

                packet.Translator.ReadXORByte(transportGuid, 2);

                tpos.X = packet.Translator.ReadSingle();
                packet.Translator.ReadUInt32("Transport Time");

                packet.Translator.ReadXORByte(transportGuid, 4);
                packet.Translator.ReadXORByte(transportGuid, 5);
                packet.Translator.ReadXORByte(transportGuid, 3);

                packet.Translator.WriteGuid("Transport GUID", transportGuid);
                packet.AddValue("Transport Position", tpos);
            }

            if (hasFallData)
            {
                if (hasFallDirection)
                {
                    packet.Translator.ReadSingle("Fall Cos");
                    packet.Translator.ReadSingle("Fall Sin");
                    packet.Translator.ReadSingle("Horizontal speed");
                }

                packet.Translator.ReadSingle("Vertical speed");
                packet.Translator.ReadUInt32("Fall time");
            }

            pos.Y = packet.Translator.ReadSingle();
            packet.Translator.ReadXORByte(guid, 7);

            if (hasTime)
                packet.Translator.ReadInt32("Timestamp");

            pos.Z = packet.Translator.ReadSingle();
            packet.Translator.ReadXORByte(guid, 3);

            for (var i = 0; i < counter; i++)
                packet.Translator.ReadInt32("IntEA");

            packet.Translator.ReadXORByte(guid, 0);

            if (hasO)
                pos.O = packet.Translator.ReadSingle();

            pos.X = packet.Translator.ReadSingle();
            packet.Translator.ReadXORByte(guid, 1);

            if (hasPitch)
                packet.Translator.ReadSingle("Pitch");

            packet.Translator.ReadSingle("Speed");
            packet.Translator.ReadXORByte(guid, 5);

            if (bitA8)
                packet.Translator.ReadInt32("IntA8");

            if (hasSplineElevation)
                packet.Translator.ReadSingle("Spline elevation");

            packet.Translator.ReadXORByte(guid, 6);

            packet.Translator.WriteGuid("Guid", guid);
            packet.AddValue("Position", pos);
        }

        [Parser(Opcode.SMSG_FLIGHT_SPLINE_SYNC)]
        public static void HandleFlightSplineSync(Packet packet)
        {
            var guid = new byte[8];

            packet.Translator.StartBitStream(guid, 0, 6, 1, 4, 5, 2, 3, 7);
            packet.Translator.ParseBitStream(guid, 7, 3, 5, 4, 2, 6, 1, 0);
            packet.Translator.ReadSingle("Duration modifier");

            packet.Translator.WriteGuid("GUID", guid);
        }

        [Parser(Opcode.SMSG_MOVE_SPLINE_SET_SWIM_SPEED)]
        public static void HandleUnknown5730(Packet packet)
        {
            var guid = new byte[8];

            packet.Translator.StartBitStream(guid, 6, 3, 4, 2, 5, 7, 0, 1);
            packet.Translator.ReadXORByte(guid, 4);
            packet.Translator.ReadSingle("Speed");
            packet.Translator.ParseBitStream(guid, 7, 1, 6, 3, 0, 2, 5);

            packet.Translator.WriteGuid("GUID", guid);
        }

        [Parser(Opcode.SMSG_MOVE_SPLINE_SET_WALK_BACK_SPEED)]
        public static void HandleSplineSetWalkSpeed(Packet packet)
        {
            var guid = new byte[8];

            packet.Translator.StartBitStream(guid, 1, 0, 6, 5, 7, 4, 3, 2);
            packet.Translator.ParseBitStream(guid, 7, 5, 1, 6, 3, 2, 4, 0);

            packet.Translator.ReadSingle("Speed");

            packet.Translator.WriteGuid("Guid", guid);
        }

        [Parser(Opcode.SMSG_TRANSFER_PENDING)]
        public static void HandleTransferPending434(Packet packet)
        {
            packet.Translator.ReadInt32<MapId>("Map ID");

            var customLoadScreenSpell = packet.Translator.ReadBit();
            var hasTransport = packet.Translator.ReadBit();
            if (hasTransport)
            {
                packet.Translator.ReadInt32<MapId>("Transport Map ID");
                packet.Translator.ReadInt32("Transport Entry");
            }

            if (customLoadScreenSpell)
                packet.Translator.ReadUInt32<SpellId>("Spell ID");
        }

        [Parser(Opcode.SMSG_SET_PLAY_HOVER_ANIM)]
        public static void HandlePlayHoverAnim(Packet packet)
        {
            var guid = new byte[8];

            packet.Translator.StartBitStream(guid, 3, 1, 6, 0, 4);
            packet.Translator.ReadBit("unk");
            packet.Translator.StartBitStream(guid, 2, 5, 7);
            packet.Translator.ParseBitStream(guid, 3, 2, 1, 4, 6, 7, 0, 5);

            packet.Translator.WriteGuid("GUID", guid);
        }

        [Parser(Opcode.SMSG_MOVE_SPLINE_DISABLE_GRAVITY)]
        public static void HandleSplineMoveGravityDisable(Packet packet)
        {
            var guid = new byte[8];

            packet.Translator.StartBitStream(guid, 0, 5, 3, 6, 1, 4, 2, 7);
            packet.Translator.ParseBitStream(guid, 6, 3, 1, 5, 4, 7, 2, 0);

            packet.Translator.WriteGuid("GUID", guid);
        }

        [Parser(Opcode.SMSG_MOVE_SPLINE_ROOT)]
        public static void HandleSplineMoveRoot(Packet packet)
        {
            var guid = new byte[8];

            packet.Translator.StartBitStream(guid, 4, 2, 5, 1, 0, 7, 6, 3);
            packet.Translator.ParseBitStream(guid, 7, 5, 3, 0, 6, 1, 4, 2);

            packet.Translator.WriteGuid("GUID", guid);
        }

        [Parser(Opcode.CMSG_UNKNOWN_1815)]
        public static void HandleUnknown1815(Packet packet)
        {
            var guid1 = new byte[8];
            var guid2 = new byte[8];

            var bit5C = false;
            var bit64 = false;
            var bit88 = false;

            var pos = new Vector4();

            pos.Z = packet.Translator.ReadSingle();
            pos.X = packet.Translator.ReadSingle();
            pos.Y = packet.Translator.ReadSingle();

            packet.Translator.StartBitStream(guid1, 1, 3);
            var bit18 = !packet.Translator.ReadBit();
            guid1[0] = packet.Translator.ReadBit();
            var hasOrientation = !packet.Translator.ReadBit();
            guid1[4] = packet.Translator.ReadBit();
            var bit70 = !packet.Translator.ReadBit();
            guid1[2] = packet.Translator.ReadBit();
            var bit95 = packet.Translator.ReadBit();
            var bit94 = packet.Translator.ReadBit();
            var bits98 = packet.Translator.ReadBits(22);
            var bit8C = packet.Translator.ReadBit();
            packet.Translator.StartBitStream(guid1, 5, 7);
            var bit90 = !packet.Translator.ReadBit();
            var bit68 = packet.Translator.ReadBit();
            var bitAC = packet.Translator.ReadBit();
            var bitA8 = !packet.Translator.ReadBit();
            var hasTime = !packet.Translator.ReadBit();
            var bit1C = !packet.Translator.ReadBit();
            guid1[6] = packet.Translator.ReadBit();

            if (bit68)
            {
                guid2[1] = packet.Translator.ReadBit();
                guid2[2] = packet.Translator.ReadBit();
                guid2[6] = packet.Translator.ReadBit();
                bit64 = packet.Translator.ReadBit();
                guid2[3] = packet.Translator.ReadBit();
                bit5C = packet.Translator.ReadBit();
                guid2[0] = packet.Translator.ReadBit();
                guid2[7] = packet.Translator.ReadBit();
                guid2[4] = packet.Translator.ReadBit();
                guid2[5] = packet.Translator.ReadBit();
            }

            var bits18 = 0u;
            if (bit18)
                bits18 = packet.Translator.ReadBits(30);

            var bits1C = 0u;
            if (bit1C)
                bits1C = packet.Translator.ReadBits(13);

            if (bit8C)
                bit88 = packet.Translator.ReadBit();

            packet.Translator.ReadXORByte(guid1, 3);
            packet.Translator.ReadXORByte(guid1, 4);
            packet.Translator.ReadXORByte(guid1, 7);
            packet.Translator.ReadXORByte(guid1, 1);
            packet.Translator.ReadXORByte(guid1, 2);
            packet.Translator.ReadXORByte(guid1, 0);
            packet.Translator.ReadXORByte(guid1, 5);

            for (var i = 0; i < bits98; ++i)
                packet.Translator.ReadInt32("IntED");

            packet.Translator.ReadXORByte(guid1, 6);

            if (bit68)
            {
                packet.Translator.ReadXORByte(guid2, 6);
                packet.Translator.ReadXORByte(guid2, 2);
                packet.Translator.ReadSingle("Float44");
                packet.Translator.ReadXORByte(guid2, 5);
                packet.Translator.ReadInt32("Int54");
                packet.Translator.ReadSingle("Float40");
                packet.Translator.ReadXORByte(guid2, 1);
                packet.Translator.ReadXORByte(guid2, 3);
                if (bit64)
                    packet.Translator.ReadInt32("Int60");
                packet.Translator.ReadXORByte(guid2, 4);
                packet.Translator.ReadSingle("Float48");
                packet.Translator.ReadXORByte(guid2, 0);
                packet.Translator.ReadXORByte(guid2, 7);
                packet.Translator.ReadSingle("Float4C");
                if (bit5C)
                    packet.Translator.ReadInt32("Int58");
                packet.Translator.ReadByte("Byte50");
                packet.Translator.WriteGuid("Guid7", guid2);
            }

            if (bit90)
                packet.Translator.ReadSingle("Float90");

            if (bit8C)
            {
                packet.Translator.ReadInt32("Int74");
                if (bit88)
                {
                    packet.Translator.ReadSingle("Float84");
                    packet.Translator.ReadSingle("Float80");
                    packet.Translator.ReadSingle("Float7C");
                }

                packet.Translator.ReadSingle("Float78");
            }

            if (bit70)
                packet.Translator.ReadSingle("Float70");
            if (hasTime)
                packet.Translator.ReadInt32("Time");
            if (bitA8)
                packet.Translator.ReadInt32("IntA8");
            if (hasOrientation)
                pos.O = packet.Translator.ReadSingle();

            packet.Translator.WriteGuid("Guid", guid1);
            packet.AddValue("Position", pos);
        }

        [Parser(Opcode.CMSG_UNKNOWN_2874)]
        public static void HandleUnknown2874(Packet packet)
        {
            var guid1 = new byte[8];
            var guid2 = new byte[8];

            var bit5C = false;
            var bit64 = false;
            var bit88 = false;

            var pos = new Vector4();

            pos.Y = packet.Translator.ReadSingle();
            pos.X = packet.Translator.ReadSingle();
            pos.Z = packet.Translator.ReadSingle();

            var bit18 = !packet.Translator.ReadBit();
            guid1[1] = packet.Translator.ReadBit();
            var hasOrientation = !packet.Translator.ReadBit();
            var bit94 = packet.Translator.ReadBit();
            packet.Translator.StartBitStream(guid1, 4, 3);
            var bit1C = !packet.Translator.ReadBit();
            var bit70 = !packet.Translator.ReadBit();
            var bit68 = packet.Translator.ReadBit();
            var bitAC = packet.Translator.ReadBit();
            guid1[7] = packet.Translator.ReadBit();
            var bits98 = packet.Translator.ReadBits(22);
            guid1[5] = packet.Translator.ReadBit();
            var bitA8 = !packet.Translator.ReadBit();
            var bit95 = packet.Translator.ReadBit();
            packet.Translator.StartBitStream(guid1, 6, 2);
            var bit90 = !packet.Translator.ReadBit();
            var hasTime = !packet.Translator.ReadBit();
            guid1[0] = packet.Translator.ReadBit();
            var bit8C = packet.Translator.ReadBit();

            var bits18 = 0u;
            if (bit18)
                bits18 = packet.Translator.ReadBits(30);

            var bits1C = 0u;
            if (bit1C)
                bits1C = packet.Translator.ReadBits(13);

            if (bit68)
            {
                guid2[3] = packet.Translator.ReadBit();
                bit5C = packet.Translator.ReadBit();
                guid2[1] = packet.Translator.ReadBit();
                guid2[0] = packet.Translator.ReadBit();
                bit64 = packet.Translator.ReadBit();
                guid2[7] = packet.Translator.ReadBit();
                guid2[5] = packet.Translator.ReadBit();
                guid2[2] = packet.Translator.ReadBit();
                guid2[4] = packet.Translator.ReadBit();
                guid2[6] = packet.Translator.ReadBit();
            }

            if (bit8C)
                bit88 = packet.Translator.ReadBit();
            packet.Translator.ReadXORByte(guid1, 1);
            packet.Translator.ReadXORByte(guid1, 2);
            packet.Translator.ReadXORByte(guid1, 6);
            for (var i = 0; i < bits98; ++i)
                packet.Translator.ReadInt32("IntED");

            packet.Translator.ReadXORByte(guid1, 3);
            packet.Translator.ReadXORByte(guid1, 4);
            packet.Translator.ReadXORByte(guid1, 7);
            packet.Translator.ReadXORByte(guid1, 0);
            packet.Translator.ReadXORByte(guid1, 5);
            if (bit68)
            {
                packet.Translator.ReadSingle("Float48");
                packet.Translator.ReadXORByte(guid2, 2);
                if (bit64)
                    packet.Translator.ReadInt32("Int60");
                packet.Translator.ReadXORByte(guid2, 6);
                packet.Translator.ReadXORByte(guid2, 1);
                if (bit5C)
                    packet.Translator.ReadInt32("Int58");
                packet.Translator.ReadXORByte(guid2, 5);
                packet.Translator.ReadByte("Byte50");
                packet.Translator.ReadXORByte(guid2, 3);
                packet.Translator.ReadXORByte(guid2, 0);
                packet.Translator.ReadSingle("Float44");
                packet.Translator.ReadSingle("Float40");
                packet.Translator.ReadXORByte(guid2, 4);
                packet.Translator.ReadXORByte(guid2, 7);
                packet.Translator.ReadSingle("Float4C");
                packet.Translator.ReadInt32("Int54");
                packet.Translator.WriteGuid("Guid7", guid2);
            }

            if (bitA8)
                packet.Translator.ReadInt32("IntA8");
            if (bit8C)
            {
                if (bit88)
                {
                    packet.Translator.ReadSingle("Float80");
                    packet.Translator.ReadSingle("Float7C");
                    packet.Translator.ReadSingle("Float84");
                }

                packet.Translator.ReadSingle("Float78");
                packet.Translator.ReadInt32("Int74");
            }

            if (bit90)
                packet.Translator.ReadSingle("Float90");
            if (hasTime)
                packet.Translator.ReadInt32("Time");
            if (bit70)
                packet.Translator.ReadSingle("Float70");
            if (hasOrientation)
                pos.O = packet.Translator.ReadSingle();

            packet.Translator.WriteGuid("Guid", guid1);
            packet.AddValue("Position", pos);
        }

        [Parser(Opcode.SMSG_MOVE_UPDATE_FLIGHT_SPEED)]
        public static void HandleMoveUpdateFlightSpeed(Packet packet)
        {
            var guid = new byte[8];

            packet.Translator.StartBitStream(guid, 7, 1, 5, 6, 4, 3, 0, 2);
            packet.Translator.ReadXORByte(guid, 0);
            packet.Translator.ReadSingle("Speed");
            packet.Translator.ParseBitStream(guid, 6, 2, 3, 1, 7, 4, 5);

            packet.Translator.WriteGuid("GUID", guid);
        }

        [Parser(Opcode.SMSG_UNKNOWN_6754)]
        public static void HandleUnknown6754(Packet packet)
        {
            var guid1 = new byte[8];
            var guid2 = new byte[8];

            var bit64 = false;
            var bit6C = false;
            var bit90 = false;

            var pos = new Vector4();

            pos.O = packet.Translator.ReadSingle();
            pos.X = packet.Translator.ReadSingle();
            packet.Translator.ReadSingle("Float10");
            pos.X = packet.Translator.ReadSingle();
            pos.Z = packet.Translator.ReadSingle();

            var hasMovementFlags = !packet.Translator.ReadBit();
            guid1[1] = packet.Translator.ReadBit();
            var bit70 = packet.Translator.ReadBit();
            guid1[0] = packet.Translator.ReadBit();
            var bit94 = packet.Translator.ReadBit();

            if (hasMovementFlags)
                packet.Translator.ReadBitsE<MovementFlag>("Movement flags", 30);

            var bitB4 = packet.Translator.ReadBit();
            if (bit70)
            {
                bit6C = packet.Translator.ReadBit();
                packet.Translator.StartBitStream(guid2, 2, 7);
                bit64 = packet.Translator.ReadBit();
                packet.Translator.StartBitStream(guid2, 5, 4, 6, 0, 1, 3);
            }

            var bit98 = !packet.Translator.ReadBit();
            var bit9C = packet.Translator.ReadBit();
            packet.Translator.StartBitStream(guid1, 3, 5, 4);
            var bit78 = !packet.Translator.ReadBit();
            var bit38 = !packet.Translator.ReadBit();
            if (bit94)
                bit90 = packet.Translator.ReadBit();
            guid1[6] = packet.Translator.ReadBit();
            var bitsA0 = packet.Translator.ReadBits(22);
            var bit9D = packet.Translator.ReadBit();
            guid1[7] = packet.Translator.ReadBit();
            var bitB0 = !packet.Translator.ReadBit();
            guid1[2] = packet.Translator.ReadBit();
            var hasExtraMovementFlags = !packet.Translator.ReadBit();
            var bit28 = !packet.Translator.ReadBit();

            if (hasExtraMovementFlags)
                packet.Translator.ReadBitsE<MovementFlagExtra>("Extra Movement Flags", 13);

            if (bit70)
            {
                packet.Translator.ReadXORByte(guid2, 0);
                packet.Translator.ReadXORByte(guid2, 6);
                if (bit64)
                    packet.Translator.ReadInt32("Int60");
                packet.Translator.ReadXORByte(guid2, 1);
                packet.Translator.ReadXORByte(guid2, 4);
                packet.Translator.ReadInt32("Int5C");
                packet.Translator.ReadSingle("Float54");
                packet.Translator.ReadXORByte(guid2, 3);
                packet.Translator.ReadSingle("Float50");
                packet.Translator.ReadByte("Byte58");
                packet.Translator.ReadXORByte(guid2, 7);
                packet.Translator.ReadXORByte(guid2, 2);
                if (bit6C)
                    packet.Translator.ReadInt32("Int68");
                packet.Translator.ReadSingle("Float4C");
                packet.Translator.ReadXORByte(guid2, 5);
                packet.Translator.ReadSingle("Float48");
                packet.Translator.WriteGuid("Guid8", guid2);
            }

            packet.Translator.ReadXORByte(guid1, 1);
            packet.Translator.ReadXORByte(guid1, 6);
            if (bit38)
                packet.Translator.ReadSingle("Float38");
            packet.Translator.ReadXORByte(guid1, 0);
            if (bit78)
                packet.Translator.ReadSingle("Float78");
            packet.Translator.ReadXORByte(guid1, 4);
            packet.Translator.ReadXORByte(guid1, 5);
            if (bit98)
                packet.Translator.ReadSingle("Float98");
            if (bit94)
            {
                packet.Translator.ReadSingle("Float80");
                packet.Translator.ReadInt32("Int7C");
                if (bit90)
                {
                    packet.Translator.ReadSingle("Float88");
                    packet.Translator.ReadSingle("Float84");
                    packet.Translator.ReadSingle("Float8C");
                }

            }

            packet.Translator.ReadXORByte(guid1, 3);
            if (bit28)
                packet.Translator.ReadInt32("Int28");
            packet.Translator.ReadXORByte(guid1, 2);

            for (var i = 0; i < bitsA0; ++i)
                packet.Translator.ReadInt32("IntEA", i);

            if (bitB0)
                packet.Translator.ReadInt32("IntB0");
            packet.Translator.ReadXORByte(guid1, 7);

            packet.Translator.WriteGuid("Guid3", guid1);
            packet.AddValue("Position", pos);
        }

        [Parser(Opcode.MSG_UNKNOWN_5750)]
        public static void HandleUnknown5750(Packet packet)
        {
            if (packet.Direction == Direction.ClientToServer)
            {
                var guid = new byte[8];

                packet.Translator.StartBitStream(guid, 1, 7, 4, 5, 3, 6, 2);
                packet.Translator.ReadBit("bit16");
                guid[0] = packet.Translator.ReadBit();
                packet.Translator.ParseBitStream(guid, 0, 2, 3, 1, 4, 6, 5, 7);

                packet.Translator.WriteGuid("Guid", guid);
            }
            else
            {
                var guid1 = new byte[8];
                var guid2 = new byte[8];

                var bit64 = false;
                var bit6C = false;
                var bit90 = false;

                var bit98 = !packet.Translator.ReadBit();
                packet.Translator.StartBitStream(guid1, 2, 6, 0, 1, 5);
                var bit28 = !packet.Translator.ReadBit();
                var bit9C = packet.Translator.ReadBit();
                var bitsA0 = packet.Translator.ReadBits(22);
                guid1[4] = packet.Translator.ReadBit();

                var bit70 = packet.Translator.ReadBit();
                if (bit70)
                {
                    packet.Translator.StartBitStream(guid2, 5, 6);
                    bit6C = packet.Translator.ReadBit();
                    packet.Translator.StartBitStream(guid2, 7, 2, 0, 4, 3);
                    bit64 = packet.Translator.ReadBit();
                    guid2[1] = packet.Translator.ReadBit();
                }

                guid1[3] = packet.Translator.ReadBit();
                var bit38 = !packet.Translator.ReadBit();
                var hasExtraMovementFlags = !packet.Translator.ReadBit();
                var bitB0 = !packet.Translator.ReadBit();
                var hasMovementFlags = !packet.Translator.ReadBit();
                guid1[7] = packet.Translator.ReadBit();
                var bit94 = packet.Translator.ReadBit();
                var bitB4 = packet.Translator.ReadBit();

                if (hasMovementFlags)
                    packet.Translator.ReadBitsE<MovementFlag>("Movement flags", 30);

                if (hasExtraMovementFlags)
                    packet.Translator.ReadBitsE<MovementFlagExtra>("Extra Movement Flags", 13);

                if (bit94)
                    bit90 = packet.Translator.ReadBit();

                var bit78 = !packet.Translator.ReadBit();
                var bit9D = packet.Translator.ReadBit();

                if (bit70)
                {
                    packet.Translator.ReadXORByte(guid2, 1);
                    packet.Translator.ReadXORByte(guid2, 5);
                    packet.Translator.ReadSingle("Float54");
                    packet.Translator.ReadSingle("Float48");
                    packet.Translator.ReadXORByte(guid2, 2);
                    packet.Translator.ReadXORByte(guid2, 3);
                    if (bit64)
                        packet.Translator.ReadInt32("Int60");
                    packet.Translator.ReadXORByte(guid2, 6);
                    packet.Translator.ReadXORByte(guid2, 0);
                    packet.Translator.ReadByte("Byte58");
                    packet.Translator.ReadSingle("Float50");
                    packet.Translator.ReadSingle("Float4C");
                    packet.Translator.ReadXORByte(guid2, 7);
                    packet.Translator.ReadXORByte(guid2, 4);
                    packet.Translator.ReadInt32("Int5C");
                    if (bit6C)
                        packet.Translator.ReadInt32("Int68");
                    packet.Translator.WriteGuid("Guid2", guid2);
                }

                if (bit38)
                    packet.Translator.ReadSingle("Float38");
                packet.Translator.ReadXORByte(guid1, 7);
                packet.Translator.ReadSingle("Float34");
                packet.Translator.ReadXORByte(guid1, 2);
                if (bit94)
                {
                    if (bit90)
                    {
                        packet.Translator.ReadSingle("Float84");
                        packet.Translator.ReadSingle("Float88");
                        packet.Translator.ReadSingle("Float8C");
                    }

                    packet.Translator.ReadSingle("Float80");
                    packet.Translator.ReadInt32("Int7C");
                }

                packet.Translator.ReadXORByte(guid1, 6);
                if (bitB0)
                    packet.Translator.ReadInt32("IntB0");
                if (bit28)
                    packet.Translator.ReadInt32("Int28");
                packet.Translator.ReadSingle("Float30");
                packet.Translator.ReadXORByte(guid1, 1);
                if (bit98)
                    packet.Translator.ReadSingle("Float98");
                packet.Translator.ReadXORByte(guid1, 4);
                if (bit78)
                    packet.Translator.ReadSingle("Float78");
                packet.Translator.ReadXORByte(guid1, 5);

                for (var i = 0; i < bitsA0; ++i)
                    packet.Translator.ReadInt32("IntEA", i);

                packet.Translator.ReadSingle("Float10");
                packet.Translator.ReadXORByte(guid1, 0);
                packet.Translator.ReadSingle("Float2C");
                packet.Translator.ReadXORByte(guid1, 3);

                packet.Translator.WriteGuid("Guid1", guid1);
            }
        }

        [Parser(Opcode.SMSG_UNKNOWN_5722)]
        public static void HandleUnknown5722(Packet packet)
        {
            var guid1 = new byte[8];
            var guid2 = new byte[8];

            var bit5C = false;
            var bit64 = false;
            var bit88 = false;

            var hasExtraMovementFlags = !packet.Translator.ReadBit();

            var hasMovementFlags = !packet.Translator.ReadBit();
            if (hasMovementFlags)
                packet.Translator.ReadBitsE<MovementFlag>("Movement flags", 30);

            packet.Translator.StartBitStream(guid1, 2, 4, 5);
            var bit95 = packet.Translator.ReadBit();
            guid1[1] = packet.Translator.ReadBit();
            var bitAC = packet.Translator.ReadBit();
            guid1[0] = packet.Translator.ReadBit();
            var bit70 = !packet.Translator.ReadBit();
            guid1[6] = packet.Translator.ReadBit();
            var bits98 = (int)packet.Translator.ReadBits(22);
            if (hasExtraMovementFlags)
                packet.Translator.ReadBitsE<MovementFlagExtra>("Extra Movement Flags", 13);

            var bit68 = packet.Translator.ReadBit();
            if (bit68)
            {
                packet.Translator.StartBitStream(guid2, 0, 1);
                bit5C = packet.Translator.ReadBit();
                packet.Translator.StartBitStream(guid2, 4, 2, 6, 5);
                bit64 = packet.Translator.ReadBit();
                packet.Translator.StartBitStream(guid2, 3, 7);
            }

            guid1[7] = packet.Translator.ReadBit();
            var bit8C = packet.Translator.ReadBit();
            var bit94 = packet.Translator.ReadBit();
            var bitA8 = !packet.Translator.ReadBit();
            if (bit8C)
                bit88 = packet.Translator.ReadBit();
            guid1[3] = packet.Translator.ReadBit();
            var bit90 = !packet.Translator.ReadBit();
            var bit20 = !packet.Translator.ReadBit();
            var bit30 = !packet.Translator.ReadBit();
            if (bit68)
            {
                packet.Translator.ReadXORByte(guid2, 7);
                packet.Translator.ReadXORByte(guid2, 2);
                packet.Translator.ReadByte("Byte50");
                if (bit5C)
                    packet.Translator.ReadInt32("Int58");
                packet.Translator.ReadXORByte(guid2, 1);
                packet.Translator.ReadSingle("Float48");
                packet.Translator.ReadXORByte(guid2, 0);
                packet.Translator.ReadXORByte(guid2, 6);
                packet.Translator.ReadXORByte(guid2, 5);
                packet.Translator.ReadSingle("Float40");
                packet.Translator.ReadSingle("Float44");
                packet.Translator.ReadSingle("Float4C");
                if (bit64)
                    packet.Translator.ReadInt32("Int60");
                packet.Translator.ReadXORByte(guid2, 4);
                packet.Translator.ReadInt32("Int54");
                packet.Translator.ReadXORByte(guid2, 3);
                packet.Translator.WriteGuid("Guid2", guid2);
            }

            if (bit70)
                packet.Translator.ReadSingle("Float70");
            if (bit20)
                packet.Translator.ReadInt32("Int20");
            if (bit8C)
            {
                packet.Translator.ReadInt32("Int74");
                packet.Translator.ReadSingle("Float78");
                if (bit88)
                {
                    packet.Translator.ReadSingle("Float7C");
                    packet.Translator.ReadSingle("Float80");
                    packet.Translator.ReadSingle("Float84");
                }
            }

            if (bitA8)
                packet.Translator.ReadInt32("IntA8");
            packet.Translator.ReadSingle("FloatB0");
            packet.Translator.ReadSingle("Float24");
            if (bit30)
                packet.Translator.ReadSingle("Float30");
            packet.Translator.ReadXORByte(guid1, 0);
            packet.Translator.ReadXORByte(guid1, 7);
            packet.Translator.ReadXORByte(guid1, 2);
            packet.Translator.ReadSingle("Float28");
            packet.Translator.ReadXORByte(guid1, 4);
            for (var i = 0; i < bits98; ++i)
                packet.Translator.ReadInt32("IntEA", i);

            if (bit90)
                packet.Translator.ReadSingle("Float90");
            packet.Translator.ReadXORByte(guid1, 6);
            packet.Translator.ReadXORByte(guid1, 3);
            packet.Translator.ReadXORByte(guid1, 1);
            packet.Translator.ReadSingle("Float2C");
            packet.Translator.ReadXORByte(guid1, 5);

            packet.Translator.WriteGuid("Guid1", guid1);
        }

        [Parser(Opcode.SMSG_UNKNOWN_6742)]
        public static void HandleUnknown6742(Packet packet)
        {
            var guid1 = new byte[8];
            var guid2 = new byte[8];

            var bit5C = false;
            var bit64 = false;
            var bit88 = false;

            var bit68 = packet.Translator.ReadBit();
            if (bit68)
            {
                packet.Translator.StartBitStream(guid2, 5, 7, 3, 6);
                bit64 = packet.Translator.ReadBit();
                packet.Translator.StartBitStream(guid2, 2, 0);
                bit5C = packet.Translator.ReadBit();
                packet.Translator.StartBitStream(guid2, 4, 1);
            }

            guid1[6] = packet.Translator.ReadBit();
            var hasExtraMovementFlags = !packet.Translator.ReadBit();
            guid1[3] = packet.Translator.ReadBit();
            var bit20 = !packet.Translator.ReadBit();
            var bit30 = !packet.Translator.ReadBit();
            var bits98 = (int)packet.Translator.ReadBits(22);
            var bit90 = !packet.Translator.ReadBit();
            guid1[2] = packet.Translator.ReadBit();
            var bit8C = packet.Translator.ReadBit();
            guid1[4] = packet.Translator.ReadBit();
            var bit94 = packet.Translator.ReadBit();
            var bitA8 = !packet.Translator.ReadBit();
            guid1[7] = packet.Translator.ReadBit();
            var bit70 = !packet.Translator.ReadBit();
            var bitAC = packet.Translator.ReadBit();
            guid1[5] = packet.Translator.ReadBit();
            if (bit8C)
                bit88 = packet.Translator.ReadBit();

            if (hasExtraMovementFlags)
                packet.Translator.ReadBitsE<MovementFlagExtra>("Extra Movement Flags", 13);

            packet.Translator.StartBitStream(guid1, 0, 1);
            var bit95 = packet.Translator.ReadBit();
            var hasMovementFlags = !packet.Translator.ReadBit();

            if (hasMovementFlags)
                packet.Translator.ReadBitsE<MovementFlag>("Movement flags", 30);

            packet.Translator.ReadXORByte(guid1, 5);
            if (bit68)
            {
                packet.Translator.ReadXORByte(guid2, 4);
                packet.Translator.ReadInt32("Int54");
                packet.Translator.ReadXORByte(guid2, 5);
                if (bit64)
                    packet.Translator.ReadInt32("Int60");
                packet.Translator.ReadSingle("Float48");
                packet.Translator.ReadXORByte(guid2, 2);
                packet.Translator.ReadXORByte(guid2, 0);
                packet.Translator.ReadSingle("Float4C");
                packet.Translator.ReadSingle("Float40");
                packet.Translator.ReadXORByte(guid2, 1);
                packet.Translator.ReadXORByte(guid2, 6);
                if (bit5C)
                    packet.Translator.ReadInt32("Int58");
                packet.Translator.ReadByte("Byte50");
                packet.Translator.ReadXORByte(guid2, 7);
                packet.Translator.ReadSingle("Float44");
                packet.Translator.ReadXORByte(guid2, 3);
                packet.Translator.WriteGuid("Guid2", guid2);
            }

            if (bit8C)
            {
                packet.Translator.ReadSingle("Float78");
                if (bit88)
                {
                    packet.Translator.ReadSingle("Float84");
                    packet.Translator.ReadSingle("Float80");
                    packet.Translator.ReadSingle("Float7C");
                }

                packet.Translator.ReadInt32("Int74");
            }

            if (bit20)
                packet.Translator.ReadInt32("Int20");

            for (var i = 0; i < bits98; ++i)
                packet.Translator.ReadInt32("IntEA", i);

            packet.Translator.ReadSingle("Float28");
            if (bit30)
                packet.Translator.ReadSingle("Float30");
            packet.Translator.ReadXORByte(guid1, 2);
            if (bit90)
                packet.Translator.ReadSingle("Float90");
            packet.Translator.ReadXORByte(guid1, 1);
            packet.Translator.ReadXORByte(guid1, 3);
            packet.Translator.ReadXORByte(guid1, 6);
            packet.Translator.ReadSingle("FloatB0");
            packet.Translator.ReadSingle("Float2C");
            if (bitA8)
                packet.Translator.ReadInt32("IntA8");
            packet.Translator.ReadXORByte(guid1, 7);
            if (bit70)
                packet.Translator.ReadSingle("Float70");
            packet.Translator.ReadSingle("Float24");
            packet.Translator.ReadXORByte(guid1, 4);
            packet.Translator.ReadXORByte(guid1, 0);

            packet.Translator.WriteGuid("Guid1", guid1);
        }

        [Parser(Opcode.SMSG_UNKNOWN_5942)]
        public static void HandleUnknown5942(Packet packet)
        {
            var guid1 = new byte[8];
            var guid2 = new byte[8];

            var bit5C = false;
            var bit64 = false;
            var bit88 = false;

            var bit20 = !packet.Translator.ReadBit();
            packet.Translator.StartBitStream(guid1, 2, 5);
            var bit90 = !packet.Translator.ReadBit();
            packet.Translator.StartBitStream(guid1, 3, 1);
            var bitA8 = !packet.Translator.ReadBit();
            guid1[6] = packet.Translator.ReadBit();
            var bit95 = packet.Translator.ReadBit();
            var bit68 = packet.Translator.ReadBit();
            guid1[7] = packet.Translator.ReadBit();

            if (bit68)
            {
                packet.Translator.StartBitStream(guid2, 5, 7, 1, 6);
                bit5C = packet.Translator.ReadBit();
                packet.Translator.StartBitStream(guid2, 0, 2, 3);
                bit64 = packet.Translator.ReadBit();
                guid2[4] = packet.Translator.ReadBit();
            }

            var bitAC = packet.Translator.ReadBit();
            var bit94 = packet.Translator.ReadBit();
            var bit30 = !packet.Translator.ReadBit();
            var bits98 = (int)packet.Translator.ReadBits(22);
            var bit8C = packet.Translator.ReadBit();
            if (bit8C)
                bit88 = packet.Translator.ReadBit();
            guid1[0] = packet.Translator.ReadBit();
            var bit70 = !packet.Translator.ReadBit();

            var hasExtraMovementFlags = !packet.Translator.ReadBit();
            if (hasExtraMovementFlags)
                packet.Translator.ReadBitsE<MovementFlagExtra>("Extra Movement Flags", 13);

            guid1[4] = packet.Translator.ReadBit();

            var hasMovementFlags = !packet.Translator.ReadBit();
            if (hasMovementFlags)
                packet.Translator.ReadBitsE<MovementFlag>("Movement flags", 30);

            if (bit68)
            {
                if (bit5C)
                    packet.Translator.ReadInt32("Int58");
                packet.Translator.ReadXORByte(guid2, 7);
                packet.Translator.ReadSingle("Float44");
                packet.Translator.ReadXORByte(guid2, 1);
                if (bit64)
                    packet.Translator.ReadInt32("Int60");
                packet.Translator.ReadSingle("Float4C");
                packet.Translator.ReadXORByte(guid2, 2);
                packet.Translator.ReadXORByte(guid2, 5);
                packet.Translator.ReadXORByte(guid2, 0);
                packet.Translator.ReadSingle("Float48");
                packet.Translator.ReadInt32("Int54");
                packet.Translator.ReadXORByte(guid2, 3);
                packet.Translator.ReadSingle("Float40");
                packet.Translator.ReadXORByte(guid2, 6);
                packet.Translator.ReadByte("Byte50");
                packet.Translator.ReadXORByte(guid2, 4);
                packet.Translator.WriteGuid("Guid2", guid2);
            }

            if (bit8C)
            {
                if (bit88)
                {
                    packet.Translator.ReadSingle("Float80");
                    packet.Translator.ReadSingle("Float84");
                    packet.Translator.ReadSingle("Float7C");
                }

                packet.Translator.ReadSingle("Float78");
                packet.Translator.ReadInt32("Int74");
            }

            packet.Translator.ReadSingle("Float2C");
            packet.Translator.ReadXORByte(guid1, 1);
            if (bit90)
                packet.Translator.ReadSingle("Float90");
            packet.Translator.ReadXORByte(guid1, 2);
            packet.Translator.ReadXORByte(guid1, 4);
            if (bit20)
                packet.Translator.ReadInt32("Int20");
            packet.Translator.ReadSingle("Float24");
            packet.Translator.ReadXORByte(guid1, 7);
            packet.Translator.ReadXORByte(guid1, 6);
            for (var i = 0; i < bits98; ++i)
                packet.Translator.ReadInt32("IntEA", i);

            packet.Translator.ReadXORByte(guid1, 0);
            if (bit70)
                packet.Translator.ReadSingle("Float70");
            if (bitA8)
                packet.Translator.ReadInt32("IntA8");
            packet.Translator.ReadXORByte(guid1, 5);
            packet.Translator.ReadSingle("FloatB0");
            if (bit30)
                packet.Translator.ReadSingle("Float30");
            packet.Translator.ReadSingle("Float28");
            packet.Translator.ReadXORByte(guid1, 3);

            packet.Translator.WriteGuid("Guid1", guid1);
        }

        [Parser(Opcode.SMSG_UNKNOWN_4890)]
        public static void HandleUnknown4890(Packet packet)
        {
            var guid1 = new byte[8];
            var guid2 = new byte[8];

            var bit94 = false;
            var bit9C = false;
            var bitC0 = false;

            var bitFC = packet.Translator.ReadBit();
            var bitA8 = !packet.Translator.ReadBit();
            var bitEC = packet.Translator.ReadBit();
            var bitF4 = packet.Translator.ReadBit();
            var bit2C = packet.Translator.ReadBit();
            guid1[0] = packet.Translator.ReadBit();
            var bit58 = !packet.Translator.ReadBit();
            var bitA0 = packet.Translator.ReadBit();
            if (bitA0)
            {
                bit94 = packet.Translator.ReadBit();
                packet.Translator.StartBitStream(guid2, 3, 4, 2);
                bit9C = packet.Translator.ReadBit();
                packet.Translator.StartBitStream(guid2, 5, 1, 6, 7, 0);
            }

            var hasExtraMovementFlags = !packet.Translator.ReadBit();
            guid1[3] = packet.Translator.ReadBit();
            var bit68 = !packet.Translator.ReadBit();
            var bitE4 = packet.Translator.ReadBit();
            var bit14 = packet.Translator.ReadBit();
            guid1[5] = packet.Translator.ReadBit();
            var bitCD = packet.Translator.ReadBit();

            if (hasExtraMovementFlags)
                packet.Translator.ReadBitsE<MovementFlagExtra>("Extra Movement Flags", 13);

            guid1[2] = packet.Translator.ReadBit();
            var bitC8 = !packet.Translator.ReadBit();
            var bit44 = packet.Translator.ReadBit();
            var bitCC = packet.Translator.ReadBit();
            var bit34 = packet.Translator.ReadBit();
            var bit3C = packet.Translator.ReadBit();
            var bitC4 = packet.Translator.ReadBit();
            guid1[1] = packet.Translator.ReadBit();
            guid1[7] = packet.Translator.ReadBit();
            guid1[4] = packet.Translator.ReadBit();
            var hasMovementFlags = !packet.Translator.ReadBit();
            var bitsD0 = (int)packet.Translator.ReadBits(22);
            guid1[6] = packet.Translator.ReadBit();

            if (hasMovementFlags)
                packet.Translator.ReadBitsE<MovementFlag>("Movement flags", 30);

            var bit104 = packet.Translator.ReadBit();
            if (bitC4)
                bitC0 = packet.Translator.ReadBit();

            var bits18 = (int)packet.Translator.ReadBits(19);
            for (var i = 0; i < bits18; ++i)
                packet.Translator.ReadBits("bitsED", 2, i);

            var bitE0 = !packet.Translator.ReadBit();
            if (bitA0)
            {
                packet.Translator.ReadXORByte(guid2, 2);
                packet.Translator.ReadXORByte(guid2, 3);
                packet.Translator.ReadXORByte(guid2, 1);
                packet.Translator.ReadByte("Byte88");
                packet.Translator.ReadSingle("Float7C");
                packet.Translator.ReadXORByte(guid2, 6);
                packet.Translator.ReadXORByte(guid2, 7);
                packet.Translator.ReadSingle("Float80");
                packet.Translator.ReadSingle("Float78");
                if (bit94)
                    packet.Translator.ReadInt32("Int90");
                packet.Translator.ReadSingle("Float84");
                packet.Translator.ReadXORByte(guid2, 4);
                packet.Translator.ReadXORByte(guid2, 0);
                packet.Translator.ReadInt32("Int8C");
                if (bit9C)
                    packet.Translator.ReadInt32("Int98");
                packet.Translator.ReadXORByte(guid2, 5);
                packet.Translator.WriteGuid("GuidE", guid2);
            }

            if (bit14)
                packet.Translator.ReadSingle("Float10");
            if (bitC4)
            {
                if (bitC0)
                {
                    packet.Translator.ReadSingle("FloatB4");
                    packet.Translator.ReadSingle("FloatBC");
                    packet.Translator.ReadSingle("FloatB8");
                }

                packet.Translator.ReadSingle("FloatB0");
                packet.Translator.ReadInt32("IntAC");
            }

            for (var i = 0; i < bits18; ++i)
            {
                packet.Translator.ReadSingle("FloatED", i);
                packet.Translator.ReadSingle("FloatED", i);
                packet.Translator.ReadSingle("FloatED", i);
                packet.Translator.ReadInt32("IntED", i);
                packet.Translator.ReadInt32("Int1C", i);
                packet.Translator.ReadSingle("FloatED", i);
            }

            if (bit68)
                packet.Translator.ReadSingle("Float68");
            packet.Translator.ReadXORByte(guid1, 5);
            if (bit34)
                packet.Translator.ReadSingle("Float30");

            for (var i = 0; i < bitsD0; ++i)
                packet.Translator.ReadInt32("IntEA", i);

            packet.Translator.ReadSingle("Float5C");
            packet.Translator.ReadXORByte(guid1, 1);
            if (bitEC)
                packet.Translator.ReadSingle("FloatE8");
            if (bitF4)
                packet.Translator.ReadSingle("FloatF0");
            if (bit2C)
                packet.Translator.ReadSingle("Float28");
            packet.Translator.ReadXORByte(guid1, 3);
            if (bit3C)
                packet.Translator.ReadSingle("Float38");
            packet.Translator.ReadXORByte(guid1, 7);
            packet.Translator.ReadSingle("Float60");
            if (bitE0)
                packet.Translator.ReadInt32("IntE0");
            if (bit58)
                packet.Translator.ReadInt32("Int58");
            packet.Translator.ReadSingle("Float64");
            if (bitC8)
                packet.Translator.ReadSingle("FloatC8");
            packet.Translator.ReadXORByte(guid1, 6);
            packet.Translator.ReadXORByte(guid1, 0);
            packet.Translator.ReadXORByte(guid1, 2);
            if (bitFC)
                packet.Translator.ReadSingle("FloatF8");
            if (bitA8)
                packet.Translator.ReadSingle("FloatA8");
            packet.Translator.ReadXORByte(guid1, 4);
            if (bit104)
                packet.Translator.ReadSingle("Float100");
            if (bit44)
                packet.Translator.ReadSingle("Float40");

            packet.Translator.WriteGuid("Guid9", guid1);
        }

        [Parser(Opcode.MSG_UNKNOWN_6127)]
        public static void HandleUnknown6127(Packet packet)
        {
            if (packet.Direction == Direction.ClientToServer)
            {
                var guid = new byte[8];

                packet.Translator.StartBitStream(guid, 5, 3, 0, 6, 1, 7, 4, 2);
                packet.Translator.ParseBitStream(guid, 6, 7, 3, 0, 5, 4, 2, 1);

                packet.Translator.WriteGuid("Guid", guid);
            }
            else
            {
                var guid1 = new byte[8];
                var guid2 = new byte[8];

                var bit5C = false;
                var bit64 = false;
                var bit88 = false;

                var bitA8 = !packet.Translator.ReadBit();
                packet.Translator.StartBitStream(guid1, 1, 5);
                var bits98 = packet.Translator.ReadBits(22);
                packet.Translator.StartBitStream(guid1, 2, 4);
                var bit90 = !packet.Translator.ReadBit();
                var bit95 = packet.Translator.ReadBit();
                var bit70 = !packet.Translator.ReadBit();
                guid1[0] = packet.Translator.ReadBit();
                var bit30 = !packet.Translator.ReadBit();
                var bit20 = !packet.Translator.ReadBit();
                var bit94 = packet.Translator.ReadBit();
                packet.Translator.StartBitStream(guid1, 7, 6);
                var bitAC = packet.Translator.ReadBit();
                guid1[3] = packet.Translator.ReadBit();

                var bit68 = packet.Translator.ReadBit();
                if (bit68)
                {
                    packet.Translator.StartBitStream(guid2, 4, 2, 7, 6, 3);
                    bit64 = packet.Translator.ReadBit();
                    guid2[1] = packet.Translator.ReadBit();
                    bit5C = packet.Translator.ReadBit();
                    packet.Translator.StartBitStream(guid2, 0, 5);
                }

                var bit8C = packet.Translator.ReadBit();

                var hasExtraMovementFlags = !packet.Translator.ReadBit();
                if (hasExtraMovementFlags)
                    packet.Translator.ReadBitsE<MovementFlagExtra>("Extra Movement Flags", 13);

                if (bit8C)
                    bit88 = packet.Translator.ReadBit();

                var hasMovementFlags = !packet.Translator.ReadBit();
                if (hasMovementFlags)
                    packet.Translator.ReadBitsE<MovementFlag>("Movement flags", 30);

                if (bit8C)
                {
                    packet.Translator.ReadInt32("Int74");

                    if (bit88)
                    {
                        packet.Translator.ReadSingle("Float80");
                        packet.Translator.ReadSingle("Float84");
                        packet.Translator.ReadSingle("Float7C");
                    }

                    packet.Translator.ReadSingle("Float78");
                }

                if (bit68)
                {
                    if (bit64)
                        packet.Translator.ReadInt32("Int60");

                    packet.Translator.ReadXORByte(guid2, 4);
                    packet.Translator.ReadSingle("Float4C");
                    packet.Translator.ReadSingle("Float44");
                    packet.Translator.ReadXORByte(guid2, 7);
                    packet.Translator.ReadXORByte(guid2, 3);
                    packet.Translator.ReadXORByte(guid2, 5);
                    packet.Translator.ReadSingle("Float40");
                    packet.Translator.ReadXORByte(guid2, 2);
                    packet.Translator.ReadSingle("Float48");
                    packet.Translator.ReadByte("Byte50");
                    packet.Translator.ReadXORByte(guid2, 6);
                    packet.Translator.ReadInt32("Int54");
                    if (bit5C)
                        packet.Translator.ReadInt32("Int58");
                    packet.Translator.ReadXORByte(guid2, 1);
                    packet.Translator.ReadXORByte(guid2, 0);
                    packet.Translator.WriteGuid("Guid2", guid2);
                }

                packet.Translator.ReadSingle("Float24");
                if (bit20)
                    packet.Translator.ReadInt32("Int20");
                if (bit70)
                    packet.Translator.ReadSingle("Float70");
                packet.Translator.ReadXORByte(guid1, 2);
                if (bit90)
                    packet.Translator.ReadSingle("Float90");
                packet.Translator.ReadXORByte(guid1, 6);
                packet.Translator.ReadXORByte(guid1, 0);
                packet.Translator.ReadXORByte(guid1, 1);
                packet.Translator.ReadXORByte(guid1, 3);
                packet.Translator.ReadXORByte(guid1, 5);
                if (bitA8)
                    packet.Translator.ReadInt32("IntA8");
                packet.Translator.ReadSingle("Float28");
                if (bit30)
                    packet.Translator.ReadSingle("Float30");
                packet.Translator.ReadXORByte(guid1, 7);
                packet.Translator.ReadXORByte(guid1, 4);
                for (var i = 0; i < bits98; ++i)
                    packet.Translator.ReadInt32("IntEA", i);

                packet.Translator.ReadSingle("Float2C");

                packet.Translator.WriteGuid("Guid1", guid1);
            }
        }
    }
}
