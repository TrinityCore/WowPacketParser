using WowPacketParser.Enums;
using WowPacketParser.Misc;
using WowPacketParser.Parsing;

namespace WowPacketParserModule.V6_0_2_19033.Parsers
{
    public static class PetHandler
    {
        [Parser(Opcode.CMSG_QUERY_PET_NAME)]
        public static void HandlePetNameQuery(Packet packet)
        {
            packet.Translator.ReadPackedGuid128("PetID");
        }

        [Parser(Opcode.SMSG_QUERY_PET_NAME_RESPONSE)]
        public static void HandlePetNameQueryResponse(Packet packet)
        {
            packet.Translator.ReadPackedGuid128("PetID");

            var hasData = packet.Translator.ReadBit("Has Data");
            if (!hasData)
                return;

            var len = packet.Translator.ReadBits(8);
            packet.Translator.ReadBit("HasDeclined");

            const int maxDeclinedNameCases = 5;
            var declinedNameLen = new int[maxDeclinedNameCases];
            for (var i = 0; i < maxDeclinedNameCases; ++i)
                declinedNameLen[i] = (int)packet.Translator.ReadBits(7);

            for (var i = 0; i < maxDeclinedNameCases; ++i)
                packet.Translator.ReadWoWString("DeclinedNames", declinedNameLen[i], i);

            packet.Translator.ReadTime("Timestamp");
            packet.Translator.ReadWoWString("Pet name", len);
        }

        public static void ReadPetAction(Packet packet, params object[] indexes)
        {
            var action = packet.Translator.ReadUInt32();
            var spellID = action & 0xFFFFFF;
            var slot = (action >> 24);

            packet.AddValue("Action", slot, indexes);
            packet.AddValue("SpellID", StoreGetters.GetName(StoreNameType.Spell, (int)spellID), indexes);
        }

        public static void ReadPetFlags(Packet packet, params object[] indexes)
        {
            var petModeFlag = packet.Translator.ReadUInt32("PetModeAndOrders");
            var reactState = (petModeFlag >> 0) & 0xFF;
            var commandState = (petModeFlag >> 8) & 0xFF;
            var flag = petModeFlag >> 16;

            packet.AddValue("ReactState", (ReactState)reactState, indexes);
            packet.AddValue("CommandState", (CommandState)commandState, indexes);
            packet.AddValue("Flag", flag, indexes);
        }

        public static void ReadPetSpellCooldownData(Packet packet, params object[] idx)
        {
            packet.Translator.ReadInt32("SpellID", idx);
            packet.Translator.ReadInt32("Duration", idx);
            packet.Translator.ReadInt32("CategoryDuration", idx);
            packet.Translator.ReadInt16("Category", idx);
        }

        public static void ReadPetSpellHistoryData(Packet packet, params object[] idx)
        {
            packet.Translator.ReadInt32("CategoryID", idx);
            packet.Translator.ReadInt32("RecoveryTime", idx);
            packet.Translator.ReadSByte("ConsumedCharges", idx);
        }

        [Parser(Opcode.SMSG_PET_SPELLS_MESSAGE)]
        public static void HandlePetSpells(Packet packet)
        {
            packet.Translator.ReadPackedGuid128("PetGUID");
            packet.Translator.ReadInt16("CreatureFamily");
            packet.Translator.ReadInt16("Specialization");
            packet.Translator.ReadInt32("TimeLimit");
            ReadPetFlags(packet, "PetModeAndOrders");

            const int maxCreatureSpells = 10;
            for (var i = 0; i < maxCreatureSpells; i++) // Read pet / vehicle spell ids
                ReadPetAction(packet, "ActionButtons", i);

            var actionsCount = packet.Translator.ReadInt32("ActionsCount");
            var cooldownsCount = packet.Translator.ReadUInt32("CooldownsCount");
            var spellHistoryCount = packet.Translator.ReadUInt32("SpellHistoryCount");

            for (int i = 0; i < actionsCount; i++)
                ReadPetAction(packet, i, "Actions");

            for (int i = 0; i < cooldownsCount; i++)
                ReadPetSpellCooldownData(packet, i, "PetSpellCooldown");

            for (int i = 0; i < spellHistoryCount; i++)
                ReadPetSpellHistoryData(packet, i, "PetSpellHistory");
        }

        [Parser(Opcode.CMSG_PET_ACTION)]
        public static void HandlePetAction(Packet packet)
        {
            packet.Translator.ReadPackedGuid128("PetGUID");

            ReadPetAction(packet, "Action");

            packet.Translator.ReadPackedGuid128("TargetGUID");
            packet.Translator.ReadVector3("ActionPosition");
        }

        public static void ReadPetRenameData(Packet packet)
        {
            packet.Translator.ReadPackedGuid128("PetGUID");
            packet.Translator.ReadInt32("PetNumber");

            var bits20 = packet.Translator.ReadBits(8);

            var bit149 = packet.Translator.ReadBit("HasDeclinedNames");
            if (bit149)
            {
                var count = new int[5];
                for (var i = 0; i < 5; ++i)
                    count[i] = (int)packet.Translator.ReadBits(7);

                for (var i = 0; i < 5; ++i)
                    packet.Translator.ReadWoWString("DeclinedNames", count[i], i);
            }

            packet.Translator.ReadWoWString("NewName", bits20);
        }

        [Parser(Opcode.SMSG_PET_NAME_INVALID)]
        public static void HandlePetNameInvalid(Packet packet)
        {
            packet.Translator.ReadByte("Result");
            ReadPetRenameData(packet);
        }

        [Parser(Opcode.CMSG_PET_RENAME)]
        public static void HandlePetRename(Packet packet)
        {
            ReadPetRenameData(packet);
        }

        [Parser(Opcode.SMSG_PET_SPECIALIZATION)]
        public static void HandlePetSpecialization(Packet packet)
        {
             packet.Translator.ReadInt16("Specialization");
        }

        [Parser(Opcode.CMSG_LEARN_PET_SPECIALIZATION_GROUP)]
        public static void HandlePetSetSpecialization(Packet packet)
        {
            packet.Translator.ReadPackedGuid128("PetGUID");
            packet.Translator.ReadInt32("SpecGroupId");
        }

        [Parser(Opcode.SMSG_PET_LEARNED_SPELLS)]
        [Parser(Opcode.SMSG_PET_UNLEARNED_SPELLS)]
        public static void HandlePetSpellsLearnedRemoved(Packet packet)
        {
            var count = packet.Translator.ReadUInt32("Spell Count");

            for (var i = 0; i < count; ++i)
                packet.Translator.ReadInt32<SpellId>("Spell ID", i);
        }

        [Parser(Opcode.SMSG_PET_GUIDS)]
        public static void HandlePetGuids(Packet packet)
        {
            var count = packet.Translator.ReadInt32("Count");
            for (var i = 0; i < count; ++i)
                packet.Translator.ReadPackedGuid128("PetGUID", i);
        }

        [Parser(Opcode.CMSG_DISMISS_CRITTER)]
        public static void HandleDismissCritter(Packet packet)
        {
            packet.Translator.ReadPackedGuid128("CritterGUID");
        }

        [Parser(Opcode.SMSG_PET_ACTION_SOUND)]
        public static void HandlePetSound(Packet packet)
        {
            packet.Translator.ReadPackedGuid128("UnitGUID");
            packet.Translator.ReadInt32("Action");
        }

        [Parser(Opcode.SMSG_PET_MODE)]
        public static void HandlePetMode(Packet packet)
        {
            packet.Translator.ReadPackedGuid128("PetGUID");
            ReadPetFlags(packet, "PetMode");
        }

        [Parser(Opcode.CMSG_PET_STOP_ATTACK)]
        public static void HandlePetStopAttack(Packet packet)
        {
            packet.Translator.ReadPackedGuid128("PetGUID");
        }

        [Parser(Opcode.CMSG_PET_SET_ACTION)]
        public static void HandlePetSetAction(Packet packet)
        {
            packet.Translator.ReadPackedGuid128("PetGUID");
            packet.Translator.ReadUInt32("Index");

            ReadPetAction(packet, "Action");
        }

        public static void ReadPetStableInfo(Packet packet, params object[] indexes)
        {
            packet.Translator.ReadInt32("PetSlot", indexes);
            packet.Translator.ReadInt32("PetNumber", indexes);
            packet.Translator.ReadInt32("CreatureID", indexes);
            packet.Translator.ReadInt32("DisplayID", indexes);
            packet.Translator.ReadInt32("ExperienceLevel", indexes);
            packet.Translator.ReadByte("PetFlags", indexes);

            packet.Translator.ResetBitReader();

            var len = packet.Translator.ReadBits(8);
            packet.Translator.ReadWoWString("PetName", len, indexes);
        }

        [Parser(Opcode.SMSG_PET_STABLE_LIST)]
        public static void HandlePetStableList(Packet packet)
        {
            packet.Translator.ReadPackedGuid128("StableMaster");
            var petCount = packet.Translator.ReadInt32("PetCount");
            for (int i = 0; i < petCount; i++)
                ReadPetStableInfo(packet, i, "PetStableInfo");
        }

        [Parser(Opcode.SMSG_PET_ADDED)]
        public static void HandlePetAdded(Packet packet)
        {
            ReadPetStableInfo(packet, "PetStableInfo");
        }

        [Parser(Opcode.SMSG_PET_ACTION_FEEDBACK)]
        public static void HandlePetActionFeedback(Packet packet)
        {
            packet.Translator.ReadInt32<SpellId>("SpellID");
            packet.Translator.ReadByteE<PetFeedback>("Response");
        }
    }
}
