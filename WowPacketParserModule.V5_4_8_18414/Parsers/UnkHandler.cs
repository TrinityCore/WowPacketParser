...
namespace WowPacketParserModule.V5_4_8_18414.Parsers
{
    public static class UnkHandler
    {
        ...
        [Parser(Opcode.SMSG_UNK_027D)]
        public static void HandleSUnk027D(Packet packet)
        {
            var hasData = !packet.ReadBit("!hasData");
            if (hasData)
                packet.ReadWoWString("str", packet.ReadBits(9));
            packet.ReadInt32("unk528"); // 528
        }
        ...
        [Parser(Opcode.SMSG_UNK_0A3A)]
        public static void HandleSUnk0A3A(Packet packet)
        {
            var guid = new byte[8];
        ...
        [Parser(Opcode.SMSG_UNK_0CE1)]
        public static void HandleSUnk0CE1(Packet packet)
        {
            var guid = packet.StartBitStream(3, 1, 5, 7, 4, 6, 2, 0);
            packet.ParseBitStream(guid, 3, 5, 2, 1, 6, 7, 4, 0);
            packet.WriteGuid("Guid", guid);
        }
