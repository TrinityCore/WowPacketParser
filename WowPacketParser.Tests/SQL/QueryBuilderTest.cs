...
namespace WowPacketParser.Tests.SQL
{
    ...
    public class QueryBuilderTest
    {
        ...
        public void TestSQLUpdate()
        {
            ...
            Assert.AreEqual("UPDATE `test_data_one_p_k` SET `ID`=4, `TestInt1`=5, `TestString1`='string2' WHERE `ID` IN (1, 2);\r\n" +
                            "UPDATE `test_data_one_p_k` SET `ID`=6, `TestInt1`=7 WHERE `ID` IN (1, 2);\r\n", 
                update.Build());
        }
